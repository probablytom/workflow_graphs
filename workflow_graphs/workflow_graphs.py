class WorkflowGraph(object):
    ...
    def decide_on(condition_function):
        self.then(Decision(condition_function))

    @cascade
    def when(cond):
        if not self.currently_building_decision:
            raise BadWorkflowFormation("Can't act on a case if we're not building a decision node! You might have a \
            badly placed 'when'.")

        self.currently_building_decision.when(con)
